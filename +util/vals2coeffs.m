...
elseif ( n < cutoff )
    ...
    if ( isempty(F{n}) )
        x = util.chebpts(n);
        % Is there a better way to do this than inv?
        F{n} = inv( cos(acos(x) * (0:n-1)) ); 
    end
    coeffs = F{n} * values;
else
    % Use fast transform
    coeffs = chebtech2_vals2coeffs(values); % TODO: Remove chebfun
end

end

function coeffs = chebtech2_vals2coeffs(values)
%VALS2COEFFS   Convert values at Chebyshev points to Chebyshev coefficients.
