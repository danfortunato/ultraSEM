...
try
    ...
    for k = 1:numFiles
        ...
        if ( durations(k) > 0 )
            % Success message.
            message = sprintf('completed in %.4fs', durations(k));
        else
...
% Yay!
% if ( all(durations > 0) )
    fprintf('All benchmarks completed in %4.4fs.\n', sum(durations));
% else
...
fprintf('  Bench #%3.3d: %s ...%s', k, link, ws);       % Print to screen.
